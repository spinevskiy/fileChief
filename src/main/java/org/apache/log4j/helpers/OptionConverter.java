 ...
 */
  ...
  */
  ...
  void selectAndConfigure(URL url, String clazz, LoggerRepository hierarchy) {
   ...
   long l4wi = 0;
   try { 
	   l4w = System.getProperty("log4j.watch"); 
   } catch(Exception e) {
	   LogLog.error(e.getMessage());
   }
   if(l4w!=null)
    {
      l4wi = FileWatchdog.DEFAULT_DELAY;
       try { 
        l4wi = Long.parseLong(l4w);
        if(l4wi<minDelay) l4wi = minDelay;
       } catch (NumberFormatException e) { }
    }
   ...
   if(clazz != null) {
     ...
     if(configurator == null) {
	   ...
	   if(l4wi > 0) {
		   PropertyConfigurator.configureAndWatch(filename,l4wi);
		   return;
	   }
